...
appServices.factory('ProductsService', function($http) {
    return {
        ...
        allByPage: function(pageSize, pageIndex, term) {
            return $http.get(options.api.base_url + '/productsByPage/' + pageSize + '/' + pageIndex );
        },
         get: function(id) {
...
appServices.factory('UserService', function ($http) {
    return {
        ...
        refresh: function(token){
            console.log(token);
            return $http.post(options.api.base_url + '/user/refresh', {token:token});
        }
    }
});